// config
var API = '/api/'; // API address
var defaultPool = 'alpha1'; // Default Pool ID
...
function loadStatsData() {
	...
	$.each(data.pools, function(index, value) {
		...
		$("#networkDifficulty").text(_formatter(value.networkStats.networkDifficulty, 3, ""));
...
function loadDashboardWorkerList(walletAddress) {
    return $.ajax(API + 'pools/' + currentPool + '/miners/' + walletAddress + '/performance')
        .done(function (data) {
            var workerList = '<thead><th>Name</th><th>Average Hash Rate (Hour)</th><th>Share Rate</th></thead><tbody>';
            if (data.length > 0) {
                // Get timestamp for performance data
                var timestamp = new Date(data[0].created);
                var timeStr = timestamp.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
                var dateStr = timestamp.toLocaleDateString([], {month: 'short', day: 'numeric'});
                ...
                $.each(data[0].workers, function (index, value) {
                    ...
                    workerList += '<td>' + _formatter(value.hashrate, 5, 'H/s') + ' (' + timeStr + ', ' + dateStr + ')</td>';
...
function loadMinersList() {
    return $.ajax(API + 'pools/' + currentPool + '/miners')
        .done(function (data) {
            var minerList = '<thead><tr><th>Address</th><th>Highest Hash Rate (24h)</th><th>Share Rate</th></tr></thead><tbody>';
            if (data.length > 0) {
                $.each(data, function (index, value) {
                    minerList += '<tr>';
...
function loadPaymentsList() {
    ...
    .done(function(data) {
        var paymentList = '<thead><tr><th>Date &amp; Time</th><th>Address</th><th>Amount</th><th>Status</th><th>Transaction</th></tr></thead><tbody>';

        if (data.length > 0) {
            $.each(data, function(index, value) {
                var createDate = convertLocalDateToUTCDate(new Date(value.created), false);
                var isAlphaPending = currentPool.startsWith('alpha') && (!value.transactionConfirmationData || value.transactionConfirmationData === '');
                var rowClass = isAlphaPending ? 'payment-pending' : 'payment-confirmed';

                // Start row with appropriate class
                paymentList += '<tr class="' + rowClass + '">';
                ...
                if (isAlphaPending) {
                    paymentList += '<td><span class="badge badge-warning">Pending</span></td>';
                    paymentList += '<td>Queued for external processing</td>';
                } else {
                    paymentList += '<td><span class="badge badge-success">Confirmed</span></td>';

                    // Only show transaction link for confirmed payments
                    if (value.transactionConfirmationData && value.transactionConfirmationData !== '') {
                        paymentList += '<td><a href="' + value.transactionInfoLink + '" target="_blank">' +
                            value.transactionConfirmationData.substring(0, 16) + ' &hellip; ' +
                            value.transactionConfirmationData.substring(value.transactionConfirmationData.length - 16) + '</a></td>';
                    } else {
                        paymentList += '<td>N/A</td>';
                    }
                }
            ...
            paymentList += '<tr><td colspan="5">No Payments Made Yet</td></tr>';
        }
